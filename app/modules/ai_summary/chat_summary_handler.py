...
class ChatSummaryHandler():
    ...
    async def process_chat_summary_request(self, nlp_response: SummaryNLPExtractedData, request_data):
        try:
            ...
            request_data = {**request_data, **await ai_system_configurator.prepare_chat_summary_config()}
    ...
    async def merge_segments_into_summary(self, segments: list[dict], nlp_response: SummaryNLPExtractedData, request_data: dict) -> dict:
        """Sends all segments to OpenAI to produce one merged cohesive summary."""
        try:
            payload_segments = [
            ...
            logger.error(f"Error in merge_segments_into_summary() :: {e}, agentid :: {request_data.get('agentid')}")
            raise e
    ...
    async def generate_chat_summary(self, request_data):
        try:
            ...
            await utils.validate_token_limits(total_content, request_data)

            response = await openai_client.process_responses_api_call(request_data)
